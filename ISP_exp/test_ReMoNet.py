...
from ReMoNet import denoise_seq_ReMoNet
from utils import batch_psnr, batch_ssim, batch_LPIPS, init_logger_test, \
				variable_to_cv2_image, remove_dataparallel_wrapper, open_sequence, close_logger
import numpy as np

NUM_IN_FR_EXT = 5 # temporal size of patch
NUM_OUT_FR = 5 # temporal output size
	...
	for idx in range(seq_len):
		...
		if len(suffix) == 0:
			out_name = os.path.join(save_dir,\
					('n{}_ReMoNet_{}').format(sigmaval, idx) + fext)
		else:
			out_name = os.path.join(save_dir,\
					('n{}_ReMoNet_{}_{}').format(sigmaval, suffix, idx) + fext)
...
def test_ReMoNet(**args):
	...
	model_temp = ReMoNet(num_input_frames=NUM_IN_FR_EXT)
	...
	with torch.no_grad():
		# process data
		seqn, _, _ = open_sequence(args['test_path'],\
									args['gray'],\
									expand_if_needed=False,\
									max_num_fr=args['max_num_fr_per_seq'])
		seqn = torch.from_numpy(seqn).to(device)
		test_gt_path = args['test_path'].replace('noisy', 'clean')
		seq, _, _ = open_sequence(test_gt_path, \
									args['gray'], \
									expand_if_needed=False,\
									max_num_fr=args['max_num_fr_per_seq'])
		...
		noise_sigma_mapped = np.power((np.power(args['noise_sigma'],2)*2), 1/2) 
		noisestd = torch.FloatTensor([noise_sigma_mapped]).to(device)

		denframes = denoise_seq_ReMoNet(seq=seqn,\
										noise_std=noisestd,\
	...
	lpips = batch_LPIPS(denframes, seq)
	...
	logger.info("\tSSIM result {:.4f}".format(ssim))
	logger.info("\tLPIPS result {:.4f}".format(lpips))
...
if __name__ == "__main__":
	# Parse arguments
	parser = argparse.ArgumentParser(description="Denoise a sequence with ReMoNet")
	parser.add_argument("--model_file", type=str,\
						default="./logs/logs_ReMoNet/net.pth", \
						help='path to model of the pretrained denoiser')
	parser.add_argument("--test_path", type=str, default="./data/derf_540p_seqs/sunflower", \
	...
	print("\n### Testing ReMoNet model ###")
	print("> Parameters:")
	for p, v in zip(argspar.__dict__.keys(), argspar.__dict__.values()):
		print('\t{}: {}'.format(p, v))
	print('\n')

	test_ReMoNet(**vars(argspar))
