...
from dataloaders import train_dali_loader, video_h5_dataset
...
def main(**args):
	...
	dataset_val_gt = ValDataset(valsetdir=args['valset_dir_gt'], gray_mode=False)
	dataset = video_h5_dataset(root=args['trainset_dir'], crop_size=args['patch_size'], sequence_length=args['temp_patch_size'])
	loader_train = DataLoader(dataset, batch_size=args['batch_size'], shuffle=True, num_workers=4)
	...
	for epoch in range(start_epoch, args['epochs']):
		...
		for i, data in enumerate(loader_train, 0):
			...
			gt_noisy = torch.cat([data[0], data[1]], dim=0)
			gt_noisy_train = normalize_augment(gt_noisy, ctrl_fr_idx)[0]
			bsz = data[0].shape[0]
			gt_train, img_train = gt_noisy_train[:bsz, ...], gt_noisy_train[bsz:, ...]
			...
			gt_train = gt_train.contiguous().cuda(non_blocking=True)
			imgn_train = imgn_train.contiguous().cuda(non_blocking=True)

			noise_map = stdn.view(N, 1, 1, 1).expand((N, 1, H, W)).contiguous().cuda(non_blocking=True) # one channel per image
			noise_map = noise_map.to(torch.float32)
...
if __name__ == "__main__":
	...
	parser.add_argument("--trainset_dir", type=str, default="/data1/xiangliuyu/fastdvdnet_data/train128000_0_1_0_1_uniform.h5", \
					 help='path of trainset')
	parser.add_argument("--valset_dir", type=str, default="./data/derf_540p_seqs_noisy/0_02/", \
						 help='path of validation set')
	parser.add_argument("--valset_dir_gt", type=str, default="./data/derf_540p_seqs_clean/0_02/", \
						 help='path of validation set')
