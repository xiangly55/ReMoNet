...
def normalize_augment_multiOut(datain, ctrl_fr_idx, num_out_frames):
	'''Normalizes and augments an input patch of dim [N, num_frames, C. H, W] in [0., 255.] to \
		[N, num_frames*C. H, W] in  [0., 1.]. It also returns the central frame of the temporal \
		patch as a ground truth.
	'''
	...
	# extract ground truth (central frame)
	start_frame = ctrl_fr_idx - num_out_frames // 2
	gt_train = img_train[:, 3*start_frame:3*(start_frame+num_out_frames), :, :]
	return img_train, gt_train


def log_running_scripts(log_dir, file_name):
	
	import shutil
	method_name = file_name.split('/')[-1].split('.')[0].split('_')[-1]
	script_names = ["train_{}.py", "train_common_{}.py", "{}.py", "test_{}.py", "models_{}.py"]
	dest_path = os.path.join(log_dir, 'scripts')
	if not os.path.exists(dest_path):
		os.makedirs(dest_path)
	for script in script_names:
		script_name = script.format(method_name)
...
def svd_orthogonalization(lyr):
	...
	if classname.find('Conv') != -1 and classname.find('3d') == -1 and classname.find('CondConv') == -1 \
							and classname.find('SepConv') == -1:
...
def asy_noise_loss(noise_map_gt, noise_map_est, alpha=0.3):
	mask = noise_map_est < noise_map_gt
	weighted_mask = torch.abs(alpha - mask.float())
	mse_loss = F.mse_loss(noise_map_est, noise_map_gt, reduction='none')
	mse_loss = torch.mean(weighted_mask * mse_loss)
	return mse_loss
